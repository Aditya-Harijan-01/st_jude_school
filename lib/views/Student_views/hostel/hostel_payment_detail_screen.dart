...
import 'package:st_jude_school/providers/student/hostel_provider.dart';
...
class HostelPaymentDetailPage extends StatefulWidget {
  final String selectedFromYear;
  final String selectedToYear;
  final String roomId;
  final List<String> selectedFeeItems;
  final double totalAmount;
  final HostelProvider? hostelProvider;

  const HostelPaymentDetailPage({
    super.key,
    required this.selectedFromYear,
    required this.selectedToYear,
    required this.roomId,
    required this.selectedFeeItems,
    required this.totalAmount,
    this.hostelProvider,
  });

  @override
  State<HostelPaymentDetailPage> createState() =>
      _HostelPaymentDetailPageState();
}

class _HostelPaymentDetailPageState extends State<HostelPaymentDetailPage> {
  ...
  Future<void> _handlePaymentVerification(response) async {
    try {
      ...
      final verificationResult =
          await PaymentVerificationService.confirmHostelPayment(
      ...
      if (verificationResult != null) {
        ...
        _showVerificationFailureDialog(response, {
          'message': 'No response from verification server',
        });
      }
    } catch (e) {
      setState(() {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Your payment was successful but there was an issue with verification.',
              ),
              ...
              SizedBox(height: screenHeight * 0.015),
              Text(
                'Please contact support with your payment ID if needed.',
  ...
  String _formatHeadIds() {
    if (widget.hostelProvider == null) {
      return '';
    }

    List<String> formattedHeadIds = [];
    final fees = widget.hostelProvider!.hostelFeeStructureResponse?.data ?? [];

    for (String itemString in widget.selectedFeeItems) {
      String targetGroupId = itemString;
      String? targetHeadId;
      ...
      if (itemString.contains('~')) {
        ...
        if (parts.length >= 2) {
          ...
          targetHeadId = parts[1];
        }
      }

      try {
        final fee = fees.firstWhere((f) {
          final matchesGroup = f.feeGroupId == targetGroupId;
          final matchesHead =
              targetHeadId == null || f.feeHeadId == targetHeadId;
          return matchesGroup && matchesHead;
        });

        if (fee.feeGroupId.isNotEmpty) {
          String headId = fee.feeHeadId.replaceAll(',', '^');
          formattedHeadIds.add('${fee.feeGroupId}_$headId');
        }
      } catch (e) {
        log('Hostel Fee not found: $itemString');
      }
    }
  ...
  Future<void> _initiatePayment() async {
    ...
    try {
      ...
      final feeHeadIds = _formatHeadIds();

      final response = await getInitiateHostalPayment(
        regNo: authProvider.loginData!.regno,
        fromYear: widget.selectedFromYear,
        toYear: widget.selectedToYear,
        roomId: widget.roomId,
        feeHeadIds: feeHeadIds,
        hostelDate: currentDate,
      );
  ...
  Future<void> _startPayment() async {
    ...
    try {
      ...
      log('payment:');
      print(_paymentData);
      print(widget.selectedFeeItems);
      log('selected fees:');
      // For hostel payment, we might not need installmentIds for razorpay call if not used locally
      // But _razorpayService.startPaymentWithApiData usually expects it for some logging or metadata if needed
      // Here we just pass empty list if not strictly required by the service logic for metadata,
      // or pass our selectedFeeItems.
      await _razorpayService.startPaymentWithApiData(
        paymentData: data,
        installmentIds: widget.selectedFeeItems,
      );
    } catch (e) {
      setState(() {
        _isPaymentProcessing = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _isVerifying
          ? null
          : AppBar(
              title: Text("Hostel Payment Detail"),
              titleTextStyle: TextStyle(fontSize: screenHeight * 0.025),
  ...
  Widget _buildFeeDetailsSection(Map<String, dynamic> data) {
    final feeHeadDescription =
        data['fee_head_description'] as List<dynamic>? ?? [];
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
  ...
  Widget _buildReceiptFooter(Map<String, dynamic> data) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            'Please wait after successful payment for Hostel Receipt.',
            style: TextStyle(
              fontSize: screenHeight * 0.015,
              fontWeight: FontWeight.w500,
              color: Colors.red,
  ...
  Widget _buildProceedButton(Map<String, dynamic> data) {
    ...
    return Container(
      ...
      child: ElevatedButton(
        onPressed: (_isPaymentProcessing || _isVerifying)
            ? null
            : _startPayment,
        // onPressed: () => {},
  ...
  Widget _buildDuplicatePaymentDialog() {
    final screenWidth = MediaQuery.of(context).size.width;
    final message =
        _paymentData!['is_duplicate_payment_message'] ??
        'Duplicate payment detected. Please contact support.';
