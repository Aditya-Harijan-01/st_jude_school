...
import 'utils/get_initial_payment.dart';
import 'utils/payment_date_utils.dart';
import 'utils/verification.dart';
import 'widgets/success_payment.dart';


enum PaymentType { fee, fine }

class PaymentDetailPage extends StatefulWidget {
  final String? selectedFromYear;
  final String? selectedToYear;

  final FeeStructureProvider? feeStructureProvider;
  ...
  const PaymentDetailPage({
    ...
    this.paymentType = PaymentType.fee,
...
class _PaymentDetailPageState extends State<PaymentDetailPage> {
  ...
  Future<void> _handlePaymentVerification(response) async {
    try {
      ...
      final verificationResult = await PaymentVerificationService.confirmFeePayment(
        regno: authProvider.loginData!.regno,
        fromyear: widget.selectedFromYear!,
        toyear:  widget.selectedToYear!,
  ...
  String _formatHeadIds() {
    if (widget.paymentType == PaymentType.fine) {
      return widget.selectedInstallmentIds?.join(',') ?? '';
    }

    if (widget.feeStructureProvider == null) {
      return '';
    }

    List<String> formattedHeadIds = [];

    // Find installments by feeGroupId in both paid and unpaid lists
    for (String installmentString in widget.selectedInstallmentIds) {
      String targetGroupId = installmentString;
      String? targetHeadIds;

      if (installmentString.contains('~')) {
        final parts = installmentString.split('~');
        if (parts.length >= 2) {
          targetGroupId = parts[0];
          targetHeadIds = parts[1];
        }
      }

      try {
        // Check unpaid installments first
        final installment = widget.feeStructureProvider!.unpaidInstallments.firstWhere((inst) {
          final matchesGroup = inst.feeGroupId == targetGroupId;
          final matchesHead = targetHeadIds == null || inst.headids == targetHeadIds;
          return matchesGroup && matchesHead;
        });

        if (installment.feeGroupId.isNotEmpty) {
          String headIds = installment.headids.replaceAll(',', '^');
          formattedHeadIds.add('${installment.feeGroupId}_$headIds');
        }
      } catch (e) {
        // If not found in unpaid, try paid installments
        try {
          final installment = widget.feeStructureProvider!.paidInstallments.firstWhere((inst) {
            final matchesGroup = inst.feeGroupId == targetGroupId;
            final matchesHead = targetHeadIds == null || inst.headids == targetHeadIds;
            return matchesGroup && matchesHead;
          });
          ...
          // If not found anywhere, continue with next installment
          log('Installment not found: $installmentString');
        }
      }
    }

    return formattedHeadIds.join(',');
  }
  ...
  Future<void> _initiatePayment() async {
    ...
    try {
      ...
      if (widget.paymentType == PaymentType.fee) {
        final formattedHeadIds = _formatHeadIds();


        response = await getInitiateFeePayment(
          regNo: authProvider.loginData!.regno,
          fromYear: widget.selectedFromYear!,
          toYear:  widget.selectedToYear!,
          headId: formattedHeadIds,
        );

      } else {
        final formattedHeadIds = _formatHeadIds();
        response = await getInitiateFinePayment(
          regNo: authProvider.loginData!.regno,
          fromYear: widget.selectedFromYear!,
          toYear:  widget.selectedToYear!,
          headId: formattedHeadIds,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _isVerifying ? null : AppBar(
        title: Text("School Payment Detail"),
  ...
  Widget _buildFeeDetailsSection(Map<String, dynamic> data) {
    ...
    // bool showConcession = data['total_concession'] != null &&
    //                       data['total_concession'] != '0.00' &&
    //                       data['total_concession'] != '0';

    // bool showConcession = true;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            widget.paymentType == PaymentType.fee ? 'FEE BREAKDOWN' : 'FINE BREAKDOWN',
            style: TextStyle(
              fontSize: screenHeight * 0.018,
              fontWeight: FontWeight.bold,
              color: CustomColor.primaryColor,
          ...
          ...feeHeadDescription.map((fee) => _buildFeeItem(
            ...
            fee['part_amount']?.toString() ?? '0.00',
            fee['latefine_amount']?.toString() ?? '0.00',
            fee['concession']?.toString() ?? '0.00',
            fee['payable']?.toString() ?? '0.00',
  ...
  Widget _buildFeeItem(String name, String amount, String lateFine, String concession, String payable) {
    final screenHeight = MediaQuery.of(context).size.height;
    final screenWidth = MediaQuery.of(context).size.width;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow('Total Late Amount', '₹${data['latefine_amount']}'),
  ...
  Widget _buildReceiptFooter(Map<String, dynamic> data) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            'Please wait after successful payment for ${widget.paymentType == PaymentType.fee ? 'Fee' : 'Fine'} Receipt.',
            style: TextStyle(
              fontSize: screenHeight * 0.015,
              fontWeight: FontWeight.w500,
              color: Colors.red,
